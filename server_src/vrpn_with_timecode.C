...
int		num_dials = 0;
#ifdef INCLUDE_TIMECODE_SERVER
vrpn_Timecode_Generator * timecode_generator;
#endif
...
int setup_Timecode_Generator (char * & pch, char * line, FILE * config_file) {
	...
	if ( ( timecode_generator =	new vrpn_Timecode_Generator(s2, connection)) == NULL) {
		fprintf(stderr,"Can't create new vrpn_Timecode_Generator\n");
		return -1;
	} 
#else
	fprintf(stderr, "vrpn_server: Can't open Timecode Generator: .cfg file option not set!\n");
#endif
	return 0; // successful completion
}
...
#endif
#ifdef INCLUDE_TIMECODE_SERVER
		if (timecode_generator) {
			timecode_generator->mainloop();
		}
#endif
