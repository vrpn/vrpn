...
int vrpn_Auxiliary_Logger::register_types(void)
{
  request_logging_m_id 
       = d_connection->register_message_type("vrpn_Auxiliary_Logger Logging_request");
  report_logging_m_id 
       = d_connection->register_message_type("vrpn_Auxiliary_Logger Logging_response");
  request_logging_status_m_id
	  = d_connection->register_message_type( "vrpn_Auxiliary_Logger Logging_status_request" );
...
bool vrpn_Auxiliary_Logger::pack_log_message_of_type (vrpn_int32 type,
                               ...
                               char **remote_out_logfile_name)
{
  ...
  if (buflen != size) {
    fprintf(stderr,"vrpn_Auxiliary_Logger::unpack_log_message_from_buffer: Buffer size incorrect\n");
    return false;
  }
  ...
  if (localInNameLen > 0) {
    if ( ( (*local_in_logfile_name) = new char[localInNameLen+1]) == NULL) {
      fprintf(stderr,"vrpn_Auxiliary_Logger::unpack_log_message_from_buffer: Out of memory\n");
      return false;
    }
...
vrpn_Auxiliary_Logger_Server::vrpn_Auxiliary_Logger_Server(const char * name, vrpn_Connection * c) :
vrpn_Auxiliary_Logger(name, c)
{
  ...
  if (dropped_last_connection_m_id == -1) {
    fprintf(stderr,"vrpn_Auxiliary_Logger_Server::vrpn_Auxiliary_Logger_Server: can't register dropped last connection type\n");
    d_connection = NULL;
    return;
  }
  if (register_autodeleted_handler(dropped_last_connection_m_id,
    static_handle_dropped_last_connection, this, vrpn_ANY_SENDER)) {
    fprintf(stderr,"vrpn_Auxiliary_Logger_Server::vrpn_Auxiliary_Logger_Server: can't register dropped last connection handler\n");
    d_connection = NULL;
  }
  ...
  if (register_autodeleted_handler(request_logging_m_id,
    ...
    fprintf(stderr,"vrpn_Auxiliary_Logger_Server::vrpn_Auxiliary_Logger_Server: can't register logging request handler\n");
    d_connection = NULL;
  }
  ...
  if (register_autodeleted_handler(request_logging_status_m_id,
    ...
    fprintf(stderr,"vrpn_Auxiliary_Logger_Server::vrpn_Auxiliary_Logger_Server: can't register logging-status request handler\n");
    d_connection = NULL;
  }
...
int vrpn_Auxiliary_Logger_Server::static_handle_request_logging(void *userdata,
	vrpn_HANDLERPARAM p)
{
  vrpn_Auxiliary_Logger_Server *me = static_cast<vrpn_Auxiliary_Logger_Server *>(userdata);
  ...
  if (!me->unpack_log_message_from_buffer(p.buffer, p.payload_len,
    ...
    fprintf(stderr, "vrpn_Auxiliary_Logger_Server::static_handle_request_logging: Could not unpack buffer\n");
    return -1;
  }
  ...
  d_logging_connection(NULL)
{
  ...
  if (( connection_to_log == NULL) || (strlen(connection_to_log) == 0) ) {
    fprintf(stderr, "vrpn_Auxiliary_Logger_Server_Generic::vrpn_Auxiliary_Logger_Server_Generic: Empty logging name passed in\n");
    d_connection = NULL;
    return;
  }
  d_connection_name = new char[strlen(connection_to_log) + 1];
  if (d_connection_name == NULL) {
    fprintf(stderr, "vrpn_Auxiliary_Logger_Server_Generic::vrpn_Auxiliary_Logger_Server_Generic: Out of memory\n");
    d_connection = NULL;
    return;
  }
...
vrpn_Auxiliary_Logger_Remote::vrpn_Auxiliary_Logger_Remote(const char * name, vrpn_Connection * c) :
  vrpn_Auxiliary_Logger(name, c)
{
  ...
  if (d_connection != NULL) {
    if (register_autodeleted_handler(report_logging_m_id, handle_report_message,
        this, d_sender_id)) {
      fprintf(stderr,"vrpn_Auxiliary_Logger_Remote: can't register handler\n");
      d_connection = NULL;
    }
  } else {
    fprintf(stderr,"vrpn_Auxiliary_Logger_Remote: Can't get connection!\n");
  }
}

/* Static */
int VRPN_CALLBACK vrpn_Auxiliary_Logger_Remote::handle_report_message(void *userdata, vrpn_HANDLERPARAM p)
{
  vrpn_Auxiliary_Logger_Remote *me = static_cast<vrpn_Auxiliary_Logger_Remote *>(userdata);
  char *localInName = NULL, *localOutName = NULL, *remoteInName = NULL, *remoteOutName = NULL;

  // Attempt to unpack the names from the buffer
  if (!me->unpack_log_message_from_buffer(p.buffer, p.payload_len,
      &localInName, &localOutName, &remoteInName, &remoteOutName)) {
    fprintf(stderr, "vrpn_Auxiliary_Logger_Remote::handle_report_message: Could not unpack buffer\n");
    return -1;
  }
