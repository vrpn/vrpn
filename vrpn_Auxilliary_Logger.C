...
vrpn_Auxilliary_Logger::vrpn_Auxilliary_Logger(const char * name, vrpn_Connection * c) :
vrpn_BaseClass(name, c)
{
  vrpn_BaseClass::init();
}

int vrpn_Auxilliary_Logger::register_types(void)
{
  request_logging_m_id 
...
bool vrpn_Auxilliary_Logger::pack_log_message_of_type (vrpn_int32 type,
                               ...
                               const char *remote_out_logfile_name)
{
  ...
  if (buf == NULL) {
    fprintf(stderr, "vrpn_Auxilliary_Logger::pack_log_message_of_type(): Out of memory.\n");
    return false;
  }
  ...
  if (lil) { vrpn_buffer(bp, &bufleft, local_in_logfile_name, lil); }
  if (lol) { vrpn_buffer(bp, &bufleft, local_out_logfile_name, lol); }
  if (ril) { vrpn_buffer(bp, &bufleft, remote_in_logfile_name, ril); }
  if (rol) { vrpn_buffer(bp, &bufleft, remote_out_logfile_name, rol); }
  int ret = d_connection->pack_message(bufsize - bufleft, now, type,
                      d_sender_id, buf, vrpn_CONNECTION_RELIABLE);
  delete [] buf;
  return (ret == 0);
}
...
bool vrpn_Auxilliary_Logger::unpack_log_message_from_buffer(
                               ...
                               char **remote_out_logfile_name)
{
  ...
  if (buflen < 4 * sizeof(localInNameLen)) {
    fprintf(stderr,"vrpn_Auxilliary_Logger::unpack_log_message_from_buffer: Buffer too small for lengths.\n");
    return false;
  }
  ...
  // Make sure we have enough room in the buffer for the four sizes and also the four
  // strings of the appropriate size.  If so, allocate the space for the strings and then
  // copy them out and NULL-terminate them.  They are not NULL-terminated in the message
  // buffer.
  int size = 4 * sizeof(localInNameLen) + localInNameLen + localOutNameLen +
    remoteInNameLen + remoteOutNameLen;
  if (buflen != size) {
    fprintf(stderr,"vrpn_Auxilliary_Logger::unpack_log_message_from_buffer: Buffer size incorrect\n");
    return false;
  }
  ...
  if (localInNameLen > 0) {
    if ( ( (*local_in_logfile_name) = new char[localInNameLen+1]) == NULL) {
      fprintf(stderr,"vrpn_Auxilliary_Logger::unpack_log_message_from_buffer: Out of memory\n");
      return false;
    }
  ...
  if (localOutNameLen > 0) {
    if ( ( (*local_out_logfile_name) = new char[localOutNameLen+1]) == NULL) {
      fprintf(stderr,"vrpn_Auxilliary_Logger::unpack_log_message_from_buffer: Out of memory\n");
      return false;
    }
  ...
  if (remoteInNameLen > 0) {
    if ( ( (*remote_in_logfile_name) = new char[remoteInNameLen+1]) == NULL) {
      fprintf(stderr,"vrpn_Auxilliary_Logger::unpack_log_message_from_buffer: Out of memory\n");
      return false;
    }
  ...
  if (remoteOutNameLen > 0) {
    if ( ( (*remote_out_logfile_name) = new char[remoteOutNameLen+1]) == NULL) {
      fprintf(stderr,"vrpn_Auxilliary_Logger::unpack_log_message_from_buffer: Out of memory\n");
      return false;
    }
...
vrpn_Auxilliary_Logger_Server::vrpn_Auxilliary_Logger_Server(const char * name, vrpn_Connection * c) :
vrpn_Auxilliary_Logger(name, c)
{
  // Register a handler for the dropped last connection message.
  dropped_last_connection_m_id = d_connection->register_message_type( vrpn_dropped_last_connection );
  if (dropped_last_connection_m_id == -1) {
    fprintf(stderr,"vrpn_Auxilliary_Logger_Server::vrpn_Auxilliary_Logger_Server: can't register dropped last connection type\n");
    d_connection = NULL;
    return;
  }
  if (register_autodeleted_handler(dropped_last_connection_m_id,
    static_handle_dropped_last_connection, this, vrpn_ANY_SENDER)) {
    fprintf(stderr,"vrpn_Auxilliary_Logger_Server::vrpn_Auxilliary_Logger_Server: can't register dropped last connection handler\n");
    d_connection = NULL;
  }

  // Register a handler for the request logging message.
  if (register_autodeleted_handler(request_logging_m_id,
    static_handle_request_logging, this, d_sender_id)) {
    fprintf(stderr,"vrpn_Auxilliary_Logger_Server::vrpn_Auxilliary_Logger_Server: can't register logging request handler\n");
    d_connection = NULL;
  }
...
// logging.
void vrpn_Auxilliary_Logger_Server::handle_dropped_last_connection(void)
{
...
int vrpn_Auxilliary_Logger_Server::static_handle_dropped_last_connection(void *userdata,
	vrpn_HANDLERPARAM p)
{
...
int vrpn_Auxilliary_Logger_Server::static_handle_request_logging(void *userdata,
	vrpn_HANDLERPARAM p)
{
  vrpn_Auxilliary_Logger_Server *me = static_cast<vrpn_Auxilliary_Logger_Server *>(userdata);
  char *localInName = NULL, *localOutName = NULL, *remoteInName = NULL, *remoteOutName = NULL;

  // Attempt to unpack the names from the buffer
  if (!me->unpack_log_message_from_buffer(p.buffer, p.payload_len,
      &localInName, &localOutName, &remoteInName, &remoteOutName)) {
    fprintf(stderr, "vrpn_Auxilliary_Logger_Server::static_handle_request_logging: Could not unpack buffer\n");
    return -1;
  }

  // Call the virtual function with the strings, then clean up memory and return.
  me->handle_request_logging(localInName, localOutName, remoteInName, remoteOutName);
  if (localInName) { delete [] localInName; };
  if (localOutName) { delete [] localOutName; };
  if (remoteInName) { delete [] remoteInName; };
  if (remoteOutName) { delete [] remoteOutName; };
  return 0;
}

vrpn_Auxilliary_Logger_Server_Generic::vrpn_Auxilliary_Logger_Server_Generic(
                  const char * logger_name, const char *connection_to_log,
                  vrpn_Connection * c) :
  vrpn_Auxilliary_Logger_Server(logger_name, c),
  ...
  d_logging_connection(NULL)
{
  ...
  if (( connection_to_log == NULL) || (strlen(connection_to_log) == 0) ) {
    fprintf(stderr, "vrpn_Auxilliary_Logger_Server_Generic::vrpn_Auxilliary_Logger_Server_Generic: Empty logging name passed in\n");
    d_connection = NULL;
    return;
  }
  d_connection_name = new char[strlen(connection_to_log) + 1];
  if (d_connection_name == NULL) {
    fprintf(stderr, "vrpn_Auxilliary_Logger_Server_Generic::vrpn_Auxilliary_Logger_Server_Generic: Out of memory\n");
    d_connection = NULL;
    return;
  }
...
void vrpn_Auxilliary_Logger_Server_Generic::handle_request_logging(
                  const char *local_in_logfile_name,
                  const char *local_out_logfile_name,
                  const char *remote_in_logfile_name,
                  const char *remote_out_logfile_name)
...
void vrpn_Auxilliary_Logger_Remote::mainloop(void)
{
    if (d_connection) {
...
vrpn_Auxilliary_Logger_Remote::vrpn_Auxilliary_Logger_Remote(const char * name, vrpn_Connection * c) :
  vrpn_Auxilliary_Logger(name, c)
{
  // Register a handler for the report callback from this device,
  // if we got a connection.
  if (d_connection != NULL) {
    if (register_autodeleted_handler(report_logging_m_id, handle_report_message,
        this, d_sender_id)) {
      fprintf(stderr,"vrpn_Auxilliary_Logger_Remote: can't register handler\n");
      d_connection = NULL;
    }
  } else {
    fprintf(stderr,"vrpn_Auxilliary_Logger_Remote: Can't get connection!\n");
  }
}

/* Static */
int VRPN_CALLBACK vrpn_Auxilliary_Logger_Remote::handle_report_message(void *userdata, vrpn_HANDLERPARAM p)
{
  vrpn_Auxilliary_Logger_Remote *me = static_cast<vrpn_Auxilliary_Logger_Remote *>(userdata);
  char *localInName = NULL, *localOutName = NULL, *remoteInName = NULL, *remoteOutName = NULL;

  // Attempt to unpack the names from the buffer
  if (!me->unpack_log_message_from_buffer(p.buffer, p.payload_len,
      &localInName, &localOutName, &remoteInName, &remoteOutName)) {
    fprintf(stderr, "vrpn_Auxilliary_Logger_Remote::handle_report_message: Could not unpack buffer\n");
    return -1;
  }
