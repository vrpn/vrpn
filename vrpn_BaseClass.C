...
int vrpn_BaseClass::init(void)
{
    ...
    if ( d_connection != NULL ) {
        d_text_message_id  = d_connection->register_message_type("vrpn Text ascii type");
        if (d_text_message_id  == -1) {
	    cerr << "vrpn_BaseClass: Can't register Text type ID" << endl;
	    d_connection = NULL;
	    return -1;
        }
        d_ping_message_id  = d_connection->register_message_type("Server, are you there?");
        if (d_ping_message_id  == -1) {
	    cerr << "vrpn_BaseClass: Can't register ping type ID" << endl;
	    d_connection = NULL;
	    return -1;
        }
        d_pong_message_id  = d_connection->register_message_type("Server is here!");
