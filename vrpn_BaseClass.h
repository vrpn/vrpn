...
Things to do to base objects to convert from VRPN version 4.XX to 5.00:
	...
	Remove the vrpn_get_connection_by_name clause from the remote constructor
...
const int vrpn_MAX_BCADRS =	100;	//< Internal value for number of BaseClass addresses
...
// It is a system class, with one instance of it in existence.  Each object in the
// system registers with this class when it is constructed.  By default, this class
// prints all Warning and Error messages to stdout, prefaced by "vrpn Warning(0) from MUMBLE: ",
// where the 0 indicates the level of the message and Warning the severity, and MUMBLE the
// name of the object that sent the message.
//  The user could create their own TextPrinter, and attach whatever objects they want
// to it.
...
class vrpn_TextPrinter {
  public:
    ...
    // of the object will not cause any error condition; deletions of unregistered
    // objects will not cause errors).
    void    remove_object(vrpn_BaseClass *o);

    /// Change the level of printing for the object (sets the minimum level to print).
    // Default is Warnings and Errors of all levels.
    ...
    /// Change the ostream that will be used to print messages.  Setting a NULL ostream
  ...
  protected:
    ...
    class vrpn_TextPrinter_Watch_Entry {
	  ...
	  vrpn_TextPrinter  *me;		//< Pointer to this, because used in a static function
	  vrpn_TextPrinter_Watch_Entry	*next;	//< Pointer to the next one in the list
    };
    vrpn_TextPrinter_Watch_Entry	*d_first_watched_object;    //< Head of list of objects being watched
...
// even when a class inherits from multiple vrpn_BaseClasses because it inherits
// from multiple user-level classes.  Note that not everything in vrpnBaseClass
// should be here, because (for example) the registration of types
// should be done for each parent class.
