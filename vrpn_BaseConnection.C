...
struct vrpn_CONNECTION_LIST {
    ...
    vrpn_CONNECTION_LIST *next;  // Next on the list
};
...
vrpn_int32 vrpn_BaseConnection::register_local_service(
    ...
    const vrpn_int32 local_id )
{
    // body skipped till next commit
}
...
vrpn_int32 vrpn_BaseConnection::translate_remote_type_to_local(
    const vrpn_int32 remote_id )
{
    return registered_remote_types [remote_id] .local_id;
}

vrpn_int32 vrpn_BaseConnection::translate_remote_service_to_local(
    const vrpn_int32 remote_id )
{
    return registered_remote_services [remote_id] .local_id;
}
...
//**************************************************************************
//
// {{{ protected: system message handlers
//
//**************************************************************************
//**************************************************************************
    ...
    vrpn_HANDLERPARAM p) 
{
    ...
    if (!me->d_local_logname) {
        ...
        // The problem with implementing this is that it's over-strict:
        // clients that assume logging succeeded unless the connection was
        // dropped will be running on the wrong assumption if we later
        // change this to be a notification message.
        //if (retval == -1) {
        //    drop_connection();
        //    status = DROPPED;
        //}
...
vrpn_int32 vrpn_BaseConnection::handle_type_message(
    void *userdata,
    vrpn_HANDLERPARAM p )
{
    vrpn_BaseConnection * me = (vrpn_BaseConnection*)userdata;
...
vrpn_int32 vrpn_BaseConnection::handle_service_message(
    void *userdata,
    vrpn_HANDLERPARAM p )
{
    vrpn_BaseConnection * me = (vrpn_BaseConnection*)userdata;
