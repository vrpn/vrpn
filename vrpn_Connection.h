...
/// @brief  Type of handler for filters on logfiles is the same as connection handler
...
/// @brief Number of endpoints that a server connection can have.  Arbitrary limit.
...
/// @brief vrpn_CONTROL is the sender used for notification messages sent to the user
...
/// @brief Encapsulation of the data and methods for a single IP-based connection
/// to take care of one part of many clients talking to a single server.
///
...
class VRPN_API vrpn_Endpoint_IP : public vrpn_Endpoint {
  ...
  public:
    ...
    /// @brief Pack a message that will be sent the next time mainloop() is called.
    ///
    /// Turn off the RELIABLE flag if you want low-latency (UDP) send.
