...
//  status= TRACKER_RESETTING;
  ...
  if (vrpn_ForceDevice::connection->register_handler(plane_message_id, handle_change_message,
	  this, vrpn_ForceDevice::my_id)) {
...
// transform rotation matrix to quaternion
   gstTransformMatrix PhantomRot=phantom->getRotationMatrix();	
	...
	q_from_row_matrix(dest_quat,rot_matrix);
...
void vrpn_Phantom::mainloop(void) {
	...
	if(duration(current_time,timestamp) >= 1000000.0/update_rate) {
		...
		// Read the current PHANToM position and force
		// Put the information into pos/quat
		get_report();
		
	    //Encode the position/orientation if there is a 
		//connection
		...
		if(vrpn_ForceDevice::connection) {
			...
			if(vrpn_ForceDevice::connection->pack_message(len,timestamp,force_message_id,
				vrpn_ForceDevice::my_id, forcebuf,vrpn_CONNECTION_LOW_LATENCY)) {
				fprintf(stderr,"Phantom: cannot write message: tossing\n");
			}
		}
  //	  print_report();
...
int vrpn_Phantom::handle_change_message(void *userdata,
	vrpn_HANDLERPARAM p)
{
	vrpn_Phantom *me = (vrpn_Phantom *)userdata;
