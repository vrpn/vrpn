...
vrpn_Imager::vrpn_Imager(const char *name, vrpn_Connection *c) :
  ...
  d_nRows(0), d_nCols(0), d_nDepth(0),
  d_nChannels(0)
{
  vrpn_BaseClass::init();
}
...
{
  d_description_m_id = d_connection->register_message_type("vrpn_Imager Description");
  d_begin_frame_m_id = d_connection->register_message_type("vrpn_Imager Begin_Frame");
  d_end_frame_m_id = d_connection->register_message_type("vrpn_Imager End_Frame");
  d_discarded_frames_m_id = d_connection->register_message_type("vrpn_Imager Discarded_Frames");
  d_throttle_frames_m_id = d_connection->register_message_type("vrpn_Imager Throttle_Frames");
  d_regionu8_m_id = d_connection->register_message_type("vrpn_Imager Regionu8");
  ...
  if ((d_description_m_id == -1) ||
      ...
      (d_regionu16_m_id == -1) ||
      (d_regionf32_m_id == -1) ||
      (d_begin_frame_m_id == -1) ||
      (d_end_frame_m_id == -1) ||
      (d_throttle_frames_m_id == -1) ||
      (d_discarded_frames_m_id == -1) ) {
    return -1;
  } else {
    return 0;
  }
...
vrpn_Imager_Server::vrpn_Imager_Server(const char *name, vrpn_Connection *c,
			 vrpn_int32 nCols, vrpn_int32 nRows, vrpn_int32 nDepth) :
    vrpn_Imager(name, c),
    ...
    d_dropped_due_to_throttle(0)
{
    ...
    // back to -1.
    register_autodeleted_handler(d_throttle_frames_m_id, handle_throttle_message, this, d_sender_id);
    register_autodeleted_handler(d_connection->register_message_type(vrpn_dropped_last_connection), handle_last_drop_message, this, vrpn_ANY_SENDER);
}
		    ...
		    vrpn_float32 scale, vrpn_float32 offset)
{
  ...
  if (scale == 0) {
    fprintf(stderr,"vrpn_Imager_Server::add_channel(): Scale was zero, set to 1\n");
    scale = 1;
  }
...
bool  vrpn_Imager_Server::send_begin_frame(const vrpn_uint16 cMin, const vrpn_uint16 cMax,
			 ...
			 const struct timeval *time)
{
  ...
  if ( (rMax >= d_nRows) || (rMin > rMax) ) {
    fprintf(stderr,"vrpn_Imager_Server::send_begin_frame(): Invalid row range (%d..%d)\n", rMin, rMax);
    return false;
  }
  if ( (cMax >= d_nCols) || (cMin > cMax) ) {
    fprintf(stderr,"vrpn_Imager_Server::send_begin_frame(): Invalid column range (%d..%d)\n", cMin, cMax);
    return false;
  }
  if ( (dMax >= d_nDepth) || (dMin > dMax) ) {
    fprintf(stderr,"vrpn_Imager_Server::send_begin_frame(): Invalid depth range (%d..%d)\n", dMin, dMax);
    return false;
  }
  ...
  if (d_connection && d_connection->pack_message(len, timestamp,
                               d_begin_frame_m_id, d_sender_id, (char*)(void*)fbuf,
                               vrpn_CONNECTION_RELIABLE)) {
    fprintf(stderr,"vrpn_Imager_Server::send_begin_frame(): cannot write message: tossing\n");
    return false;
  }
...
bool  vrpn_Imager_Server::send_end_frame(const vrpn_uint16 cMin, const vrpn_uint16 cMax,
			 const vrpn_uint16 rMin, const vrpn_uint16 rMax,
			 const vrpn_uint16 dMin, const vrpn_uint16 dMax,
			 const struct timeval *time)
{
  ...
  // Make sure the region request has indices all within the image size.
  if ( (rMax >= d_nRows) || (rMin > rMax) ) {
    fprintf(stderr,"vrpn_Imager_Server::send_end_frame(): Invalid row range (%d..%d)\n", rMin, rMax);
    return false;
  }
  if ( (cMax >= d_nCols) || (cMin > cMax) ) {
    fprintf(stderr,"vrpn_Imager_Server::send_end_frame(): Invalid column range (%d..%d)\n", cMin, cMax);
    return false;
  }
  if ( (dMax >= d_nDepth) || (dMin > dMax) ) {
    fprintf(stderr,"vrpn_Imager_Server::send_end_frame(): Invalid depth range (%d..%d)\n", dMin, dMax);
    return false;
  }
  ...
  vrpn_int32  len = sizeof(fbuf) - buflen;
  if (d_connection && d_connection->pack_message(len, timestamp,
                               d_end_frame_m_id, d_sender_id, (char*)(void*)fbuf,
                               vrpn_CONNECTION_RELIABLE)) {
    fprintf(stderr,"vrpn_Imager_Server::send_end_frame(): cannot write message: tossing\n");
    return false;
  }
			 ...
			 const struct timeval *time)
{
  ...
  vrpn_int32  len = sizeof(fbuf) - buflen;
  if (d_connection && d_connection->pack_message(len, timestamp,
                               d_discarded_frames_m_id, d_sender_id, (char*)(void*)fbuf,
                               vrpn_CONNECTION_RELIABLE)) {
    fprintf(stderr,"vrpn_Imager_Server::send_discarded_frames(): cannot write message: tossing\n");
    return false;
  }
...
/** As efficiently as possible, pull the values out of the array whose pointer is passed
    ...
    the type that has been passed in (the type that data points to).
      ...
      colStride, rowStride, depthStride: Number of elements (size of data type) to skip along a column, row, and depth
      nRows, invert_rows: If invert_rows is true, then nRows must be set to the number of rows
...
bool  vrpn_Imager_Server::send_region_using_base_pointer(vrpn_int16 chanIndex,
		    vrpn_uint16 cMin, vrpn_uint16 cMax,
		    vrpn_uint16 rMin, vrpn_uint16 rMax,
		    const vrpn_uint8 *data,
		    vrpn_uint32	colStride, vrpn_uint32 rowStride, vrpn_uint16 nRows, bool invert_rows,
		    ...
		    const struct timeval *time)
{
  ...
  if ( (chanIndex < 0) || (chanIndex >= d_nChannels) ) {
    fprintf(stderr,"vrpn_Imager_Server::send_region_using_base_pointer(): Invalid channel index (%d)\n", chanIndex);
    return false;
  }
  if ( (dMax >= d_nDepth) || (dMin > dMax) ) {
    fprintf(stderr,"vrpn_Imager_Server::send_region_using_base_pointer(): Invalid depth range (%d..%d)\n", dMin, dMax);
    return false;
  }
  if ( (rMax >= d_nRows) || (rMin > rMax) ) {
    fprintf(stderr,"vrpn_Imager_Server::send_region_using_base_pointer(): Invalid row range (%d..%d)\n", rMin, rMax);
    return false;
  }
  if ( (cMax >= d_nCols) || (cMin > cMax) ) {
  ...
  if ( invert_rows && (nRows < rMax) ) {
    fprintf(stderr,"vrpn_Imager_Server::send_region_using_base_pointer(): nRows must not be less than rMax\n");
    return false;
  }
  ...
  if (d_channels[chanIndex].d_compression != vrpn_Imager_Channel::NONE) {
    fprintf(stderr,"vrpn_Imager_Server::send_region_using_base_pointer(): Compression not implemented\n");
    return false;
  }
  ...
  if (colStride == 1) {
	...
	memcpy(msgbuf, &data[d*depthStride + rActual*rowStride + cMin], linelen);
	msgbuf += linelen;
	buflen -= linelen;
      }
    }
  } else {
    if (buflen < (int)((dMax-dMin+1)*(rMax-rMin+1)*(cMax-cMin+1)*sizeof(data[0]))) {
      return false;
    }
    ...
    for (unsigned d = dMin; d <= dMax; d++) {
      ...
      const vrpn_uint8 *rowStart = &data[d*depthStride + rMin*rowStride + cMin];
      if (invert_rows) {
	rowStart = &data[d*depthStride + (nRows-1-rMin)*rowStride + cMin];
      }
  ...
  if (d_connection && d_connection->pack_message(len, timestamp,
                               ...
                               vrpn_CONNECTION_RELIABLE)) {
    fprintf(stderr,"vrpn_Imager_Server::send_region_using_base_pointer(): cannot write message: tossing\n");
    return false;
  }
...
bool  vrpn_Imager_Server::send_region_using_base_pointer(vrpn_int16 chanIndex, vrpn_uint16 cMin, vrpn_uint16 cMax,
		    ...
		    const struct timeval *time)
{
  ...
  if (colStride == 1) {
    ...
    for (unsigned d = dMin; d <= dMax; d++) {
      // XXX Turn the depth calculation into start and += like the others
      const vrpn_uint16 *rowStart = &data[d*depthStride + rMin*rowStride + cMin];
      if (invert_rows) {
	rowStart = &data[d*depthStride + (nRows-1-rMin)*rowStride + cMin];
      }
		    ...
		    const struct timeval *time)
{
  ...
  if (colStride == 1) {
    ...
    for (unsigned d = dMin; d <= dMax; d++) {
      // XXX Turn the depth calculation into start and += like the others
      const vrpn_float32 *rowStart = &data[d*depthStride + rMin*rowStride + cMin];
      if (invert_rows) {
	rowStart = &data[d*depthStride + (nRows-1-rMin)*rowStride + cMin];
      }
...
bool  vrpn_Imager_Server::send_region_using_first_pointer(vrpn_int16 chanIndex, vrpn_uint16 cMin, vrpn_uint16 cMax,
		    ...
		    const struct timeval *time)
{
  ...
  const vrpn_uint8  *new_base = data - (cMin + rowStride*rMin + depthStride*dMin);
  if (send_region_using_base_pointer(chanIndex, cMin, cMax, rMin, rMax, new_base,
      colStride, rowStride, nRows, invert_rows, depthStride, dMin, dMax, time) ) {
    return true;
  } else {
    fprintf(stderr, "vrpn_Imager_Server::send_region_using_first_pointer(): Call to send using offset base_pointer failed.\n");
    return false;
  }
}
...
bool  vrpn_Imager_Server::send_region_using_first_pointer(vrpn_int16 chanIndex, vrpn_uint16 cMin, vrpn_uint16 cMax,
		    ...
		    vrpn_uint16 nRows, bool invert_rows,
		    vrpn_uint32 depthStride, vrpn_uint16 dMin, vrpn_uint16 dMax,
		    const struct timeval *time)
{
  // If we are discarding frames, return failure to send.
  ...
  // Point the data pointer back before the first pointer, to the place it
  // should be to make the index math work out.
  const vrpn_uint16  *new_base = data - (cMin + rowStride*rMin + depthStride*dMin);
  if (send_region_using_base_pointer(chanIndex, cMin, cMax, rMin, rMax, new_base,
      colStride, rowStride, nRows, invert_rows, depthStride, dMin, dMax, time) ) {
    return true;
  } else {
		    ...
		    const struct timeval *time)
{
  ...
  // Point the data pointer back before the first pointer, to the place it
  // should be to make the index math work out.
  const vrpn_float32  *new_base = data - (cMin + rowStride*rMin + depthStride*dMin);
  if (send_region_using_base_pointer(chanIndex, cMin, cMax, rMin, rMax, new_base,
      colStride, rowStride, nRows, invert_rows, depthStride, dMin, dMax, time) ) {
    return true;
  } else {
...
bool  vrpn_Imager_Server::send_description(void)
{
  ...
  if (vrpn_buffer(&msgbuf, &buflen, d_nDepth) ||
    ...
    fprintf(stderr,"vrpn_Imager_Server::send_description(): Can't pack message header, tossing\n");
    return false;
  }
  for (i = 0; i < d_nChannels; i++) {
    if (!d_channels[i].buffer(&msgbuf, &buflen)) {
      fprintf(stderr,"vrpn_Imager_Server::send_description(): Can't pack message channel, tossing\n");
      return false;
    }
  }
  ...
  if (d_connection && d_connection->pack_message(len, timestamp,
                               ...
                               vrpn_CONNECTION_RELIABLE)) {
    fprintf(stderr,"vrpn_Imager_Server::send_description(): cannot write message: tossing\n");
    return false;
  }
...
bool  vrpn_Imager_Server::set_resolution(vrpn_int32 nCols, vrpn_int32 nRows, vrpn_int32 nDepth)
{
  if ( (nCols <= 0) || (nRows <= 0) || (nDepth <= 0) ) {
    fprintf(stderr,"vrpn_Imager_Server::set_resolution(): Invalid size (%d, %d, %d)\n", nCols, nRows, nDepth);
    return false;
  }
  d_nDepth = nDepth;
  d_nCols = nCols;
...
void  vrpn_Imager_Server::mainloop(void)
{
  server_mainloop();
}
...
int  vrpn_Imager_Server::handle_last_drop_message(void *userdata, vrpn_HANDLERPARAM)
{
  vrpn_Imager_Server  *me = (vrpn_Imager_Server*)userdata;
  ...
  d_got_description(false)
{
  ...
  register_autodeleted_handler(d_begin_frame_m_id, handle_begin_frame_message, this, d_sender_id);
  register_autodeleted_handler(d_end_frame_m_id, handle_end_frame_message, this, d_sender_id);
  register_autodeleted_handler(d_discarded_frames_m_id, handle_discarded_frames_message, this, d_sender_id);
	...
	vrpn_HANDLERPARAM p)
{
  ...
  // called for.
  if (me->d_channels[reg.d_chanIndex].d_compression != vrpn_Imager_Channel::NONE) {
    fprintf(stderr,"vrpn_Imager_Remote::handle_region_message(): Compression not implemented\n");
    return -1;
  }
	...
	vrpn_HANDLERPARAM p)
{
  ...
  if (vrpn_unbuffer(&bufptr, &bf.dMin) ||
    ...
    fprintf(stderr, "vrpn_Imager_Remote::handle_begin_frame_message(): Can't unbuffer parameters!\n");
    return -1;
  }
...
bool  vrpn_Imager_Remote::throttle_sender(vrpn_int32 N)
{
  ...
  if (vrpn_buffer(&msgbuf, &buflen, N)) {
    fprintf(stderr,"vrpn_ImagerPose_Server::throttle_sender(): Can't pack message header, tossing\n");
    return false;
  }
  ...
  vrpn_gettimeofday(&timestamp, NULL);
  if (d_connection && d_connection->pack_message(len, timestamp,
                               d_throttle_frames_m_id, d_sender_id, (char *)(void*)fbuf,
                               vrpn_CONNECTION_RELIABLE)) {
    fprintf(stderr,"vrpn_ImagerPose_Server::throttle_sender(): cannot write message: tossing\n");
    return false;
  }
...
bool  vrpn_Imager_Region::decode_unscaled_region_using_base_pointer(vrpn_uint8 *data,
    vrpn_uint32 colStride, vrpn_uint32 rowStride, vrpn_uint32 depthStride,
    vrpn_uint16 nRows, bool invert_rows, unsigned repeat) const
{
  ...
  if (d_valType == vrpn_IMAGER_VALTYPE_UINT8) {
    ...
    // Transcode from 32-bit floating-point to 8-bit values during the conversion process.
    // As this is unscaled, we do not adjust the values -- simply jam the values in and
	...
	for (unsigned c = d_cMin; c <= d_cMax; c++) {
	  for (unsigned rpt = 0; rpt < repeat; rpt++) {
	    *(copyTo+rpt) = static_cast<vrpn_uint8>(*msgbuf);  //< Copy the current element
	  }
	  msgbuf++;		    //< Skip to the next buffer location
	  copyTo += colStride;	    //< Skip appropriate number of elements
	}
...
bool vrpn_ImagerPose::compute_pixel_center(
    vrpn_float64 *center, const vrpn_Imager &image,
    vrpn_uint16 col, vrpn_uint16 row, vrpn_uint16 depth)
{
  // Make sure we don't have a NULL pointer to fill our return into.
  if (center == NULL) {
    fprintf(stderr,"vrpn_ImagerPose::compute_pixel_center(): NULL center pointer\n");
    return false;
  }
  ...
  if (	(col >= image.nCols()) || (row >= image.nRows()) ||
	(depth >= image.nDepth()) ) {
    fprintf(stderr,"vrpn_ImagerPose::compute_pixel_center(): Pixel index out of range\n");
    return false;
  }
  ...
  center[0] = d_origin[0] + (0.5 + col)*stepC*d_dCol[0] + (0.5 + row)*stepR*d_dRow[0] + (0.5 + depth)*stepD*d_dDepth[0];
  center[1] = d_origin[1] + (0.5 + col)*stepC*d_dCol[1] + (0.5 + row)*stepR*d_dRow[1] + (0.5 + depth)*stepD*d_dDepth[1];
  center[2] = d_origin[2] + (0.5 + col)*stepC*d_dCol[2] + (0.5 + row)*stepR*d_dRow[2] + (0.5 + depth)*stepD*d_dDepth[2];
...
vrpn_ImagerPose_Server::vrpn_ImagerPose_Server(
    ...
    const vrpn_float64 dRow[3], const vrpn_float64 *dDepth,
    vrpn_Connection *c) :
...
bool  vrpn_ImagerPose_Server::set_range(const vrpn_float64 origin[3], const vrpn_float64 dCol[3],
    const vrpn_float64 dRow[3], const vrpn_float64 *dDepth)
{
  memcpy(d_origin, origin, sizeof(d_origin));
  memcpy(d_dCol, dCol, sizeof(d_dCol));
...
bool  vrpn_ImagerPose_Server::send_description(void)
{
  ...
  if (vrpn_buffer(&msgbuf, &buflen, d_origin[0]) ||
    ...
    fprintf(stderr,"vrpn_ImagerPose_Server::send_description(): Can't pack message header, tossing\n");
    return false;
  }
...
int  vrpn_ImagerPose_Server::handle_ping_message(void *userdata, vrpn_HANDLERPARAM)
{
...
void  vrpn_ImagerPose_Server::mainloop(void)
{
  server_mainloop();
}
