...
#define VRPN_MAINLOOPOBJECT_MSG(_x)                                            \
    std::cout << __FILE__ << ":" << __LINE__ << ": " << _x << std::endl;
#else
#define VRPN_MAINLOOPOBJECT_MSG(_x)
#endif
...
public:
    ...
    struct CannotWrapNullPointerIntoMainloopObject : public std::logic_error {
        CannotWrapNullPointerIntoMainloopObject()
            : std::logic_error(
                  "Cannot wrap a null pointer into a vrpn_MainloopObject!")
        {
        }
    };
    ...
    template <class T> static vrpn_MainloopObject *wrap(T o);

    /// Templated wrapping function that can encourage the
    /// wrapper to not destroy the wrapped object at destruction
    template <class T> static vrpn_MainloopObject *wrap(T o, bool owner);
...
protected:
    ...
    vrpn_MainloopObject() {}
    friend bool operator==(vrpn_MainloopObject const &lhs,
...
/// @{
inline bool operator==(vrpn_MainloopObject const &lhs,
                       vrpn_MainloopObject const &rhs)
{
...
namespace detail {
    ...
    public:
        ...
        virtual ~TypedMainloopObject()
        {
            if (_do_delete) {
                ...
                VRPN_MAINLOOPOBJECT_MSG("NOT deleting contained vrpn object "
                                        << _instance)
            }
        }
        ...
        virtual bool broken() { return (_instance->connectionPtr() == NULL); }

    protected:
        virtual void *_returnContained() const { return _instance; }
        T *_instance;
        bool _do_delete;
    };
    ...
    public:
        ...
        virtual ~TypedMainloopObject()
        {
            VRPN_MAINLOOPOBJECT_MSG("Unreferencing contained vrpn connection "
                                    << _instance)
