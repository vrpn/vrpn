...
vrpn_int32 vrpn_UnreliableMulticastRecvr::handle_incoming_message(
    char* inbuf_ptr,
    const timeval* timeout)
{
...
#ifdef	VERBOSE2
    printf("vrpn_UnreliabelMulticastRecvr::handle_incoming_message()"
           " called\n");
#endif
    ...
    if (sel_ret == -1) {
        if (errno == EINTR) { /* Ignore interrupt */
            ...
            perror("vrpn: vrpn_NetConnection::handle_udp_messages:"
                   " select failed()");
            return(-1);
        }
    }
    ...
    if (FD_ISSET(get_mcast_sock(), &exceptfds)) {
        fprintf(stderr, "vrpn: vrpn_NetConnection::handle_udp_messages:"
                " Exception on socket\n");
        return(-1);
    }
...
#ifdef	VERBOSE2
	printf("vrpn_UnreliableMulticastRecvr::handle_incoming_message()"
               " something to read\n");
#endif
        // Read the buffer
        inbuf_ptr = d_mcast_inbuf;
        if ( -1 == (inbuf_len = recv(get_mcast_sock(),
                                     d_mcast_inbuf,
                                     sizeof(d_MCASTinbufToAlignRight), 0)) )
        {
            perror("vrpn: vrpn_UnreliableMulticastrecvr::"
                   "handle_incoming_message: recv() failed");
            return -1;
        }
        return inbuf_len;
    }
...
vrpn_int32 vrpn_UnreliableMulticastRecvr::join_mcast_group(char* group)
{
    ...
    if (-1 == setsockopt(get_mcast_sock(),
                         IPPROTO_IP,
                         IP_ADD_MEMBERSHIP,
                         (char *) &mreq,
                         sizeof(mreq)))
...
void vrpn_UnreliableMulticastRecvr::init_mcast_channel()
{
    ...
    if (bind(get_mcast_sock(),
             (struct sockaddr *) &d_mcast_addr,
             sizeof(d_mcast_addr))
        < 0)
    {
    ...
    if (getsockname(get_mcast_sock(),
                    (struct sockaddr *) &d_mcast_addr,
                    (int *) &addr_len))
    {
        fprintf(stderr, "vrpn: vrpn_UnreliableMulticastSender:"
