 ...
 * ROLE      : This creates a vrpn_YEI_3Space and sets it to reset mode. It opens
 *             the serial device using the code in the vrpn_Serial_Analog constructor.
 * ARGUMENTS : 
 * RETURN    : 
 ******************************************************************************/
vrpn_YEI_3Space::vrpn_YEI_3Space (const char * p_name, vrpn_Connection * p_c, const char * p_port, int p_baud, int p_mode, bool tenbytes):
  ...
  _gotInfo (false)  // used to know if we have gotten a first full wireless report.
{
  ...
  // stream sent by the YEI_3Space glove in stream mode.
...
/******************************************************************************
 * NAME      : vrpn_YEI_3Space::clear_values
 * ROLE      : 
 * ARGUMENTS : 
 * RETURN    : 
 ******************************************************************************/
void
vrpn_YEI_3Space::clear_values (void)
{
  int i;
 ...
 ******************************************************************************/
int
vrpn_YEI_3Space::send_command (const unsigned char *p_cmd, int p_len)
{
  int            l_ret;
...
vrpn_YEI_3Space::reset (void)
{
  ...
  if (_wireless) {
    ...
    if (l_ret != 1) {
      VRPN_MSG_ERROR ("vrpn_YEI_3Space: Unable to read from the glove\n");
      return -1;
    }
    if (l_inbuf[0] == 0x80) {
      ...
      VRPN_MSG_INFO ("vrpn_YEI_3Space: Got a possible header byte!");
      return 0;
    }
    return 0;
  }
  ...
  if (l_inbuf[0] != 85) {
    VRPN_MSG_ERROR ("vrpn_YEI_3Space: Cannot get response on init command");
    return -1;
  } else {
    vrpn_flush_input_buffer (serial_fd);
    ...
    if (l_ret != 32) {
      VRPN_MSG_ERROR ("vrpn_YEI_3Space: Cannot get info. from the glove");
      return -1;
    }
    if ( (l_inbuf[0] != 66) || (l_inbuf[1] != 82)) {
      VRPN_MSG_ERROR ("vrpn_YEI_3Space: Cannot get good header on info command");
      return -1;
    }

    sprintf (l_errmsg, "vrpn_YEI_3Space: glove \"%s\"version %d.%d\n", &l_inbuf [16], l_inbuf [2], l_inbuf [3]);
...
/******************************************************************************
 * NAME      : vrpn_YEI_3Space::syncing
 * ROLE      : Send the "C" command to ask for new data from the glove
 * ARGUMENTS : void
 * RETURN    : void
 ******************************************************************************/
void vrpn_YEI_3Space::syncing (void)
{
  ...
  switch (_mode)
    ...
    case 2:
      ...
      break;
    default :
      VRPN_MSG_ERROR ("vrpn_YEI_3Space::syncing : internal error : unknown state");
      printf ("mode %d\n", _mode);
      break;
    }
  _bufcount = 0;
...
/******************************************************************************
 * NAME      : vrpn_YEI_3Space::get_report
 ...
 * RETURN    : void
 ******************************************************************************/
void vrpn_YEI_3Space::get_report (void)
{
  ...
  switch (_mode) {
    ...
    case 2:           // Streaming Mode, just go back for the next report.
      ...
      break;
    default :
      VRPN_MSG_ERROR ("vrpn_YEI_3Space::get_report : internal error : unknown state");
      break;
  }
}
